...
import SellerOrdersCompletedList from './Components/SellerOrdersCompletedList'

const SellerOrdersCompletedPage = () => {
    const [value, setValue] = React.useState(7);
    const [orders, setOrders] = useState([])

    const fetchOrders = async () => {
        const data = await api.getSellerOrderListByStatusAndPage('completed', 1)
        return data.data
    }
        ...
        <div>
            ...
            <Box sx={{ flexGrow: 1, display: 'flex' }}>
                ...
                >
                    ...
                    <Tab label="已完成" disabled {...a11yProps(7)} />
                </Tabs>
                <TabPanel value={value} index={7}>
                    {orders.length > 0 ? orders.map((order, index) => (
                        //setRefunddes(order.description)
                        <SellerOrdersCompletedList order={order} />
...
export default SellerOrdersCompletedPage